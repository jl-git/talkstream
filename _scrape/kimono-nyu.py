...
API="https://www.kimonolabs.com/api/5wi1lmds?apikey=p7uaht6sJR2Ni0jKPODwSLmUT99KjEyT"
...
for rec in data['results']['collection1']:
    ...
    d = "{0} 10:00:00".format(rec['Date'])
    try:
        speaker = m.group(1)
        affil = m.group(2)
        d = dateutil.parser.parse(d, fuzzy=True)
        fname = "{0}-nyu-nasc.md".format(d.date().isoformat())
